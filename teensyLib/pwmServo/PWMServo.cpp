...
uint8_t PWMServo::attach(int pinArg, int min, int max) {
    ...
    if(pinArg != SERVO_PIN_A && pinArg != SERVO_PIN_B && pinArg != SERVO_PIN_C) {
        return 0;
    }

#else

    if(pinArg != SERVO_PIN_A && pinArg != SERVO_PIN_B) {
        return 0;
    }
    ...
    if(!attachedA && !attachedB && !attachedC) {
        seizeTimer1();
    }
...
uint8_t PWMServo::attached() {
    if(pin == SERVO_PIN_A && attachedA) {
        return 1;
    }

    if(pin == SERVO_PIN_B && attachedB) {
        return 1;
    }

#ifdef SERVO_PIN_C

    if(pin == SERVO_PIN_C && attachedC) {
        return 1;
    }
